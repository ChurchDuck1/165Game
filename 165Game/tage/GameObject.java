...
public class GameObject
{
	...
	*/
	...
	/** rotates GameObject left/right around its current world up axis 
	 * uses object's world up axis as rotation axis, pre-multiplies and
	 * updates localRotation and world transforms
	 */
	public void yaw(float amount) {
		Vector3f upAxis = this.getWorldUpVector().normalize();

		Matrix4f rotation = new Matrix4f().rotation(amount, upAxis);

		localRotation = rotation.mul(localRotation, new Matrix4f());
		update();
	}

	/** rotates GameObject left/right around the global Y axis */
	public void globalYaw(float amount) {
		Matrix4f rotation = new Matrix4f().rotation(amount, 0f, 1f, 0f);
		localRotation = rotation.mul(localRotation, new Matrix4f());
		update();
	}

	/** rotates GameObject up/down around its current world right axis
	 * uses object's world right axis as rotation axis, pre-multiplies and
	 * updates localRotation and world transforms
	 */
	public void pitch(float amount) {
		Vector3f rightAxis = this.getWorldRightVector().normalize();

		Matrix4f rotation = new Matrix4f().rotation(amount, rightAxis);
