...
public class Camera
{
	...
	/** rotates the camera left/right around its local V axis
	 * updates the U and N vectors while keeping V fixed
	 */
	public void yaw(float amount) {
		Matrix4f rotation = new Matrix4f().rotation(amount, v);

		u.mulPosition(rotation).normalize();
		n.mulPosition(rotation).normalize();;
	}

	/** rotates the camera up/down around its local U axis
	 * updates the V and N vectors while keeping U fixed
	 */
	public void pitch(float amount) {
		Matrix4f rotation = new Matrix4f().rotation(amount,u);

		v.mulPosition(rotation).normalize();
		n.mulPosition(rotation).normalize();
	}
